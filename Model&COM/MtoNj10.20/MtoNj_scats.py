...
filename = r"C:\Users\zj2187\Desktop\MtoNj\MtoNj_sig.inpx" #好像不能用相对路径
Vissim.LoadNet(filename)
Filename = r"C:\Users\zj2187\Desktop\MtoNj\MtoNj_sig.layx"
...
SH_numbers = [1,2,3,4]
total_cycle = 240
stage_lengths = [total_cycle/4 for i in range(len(SH_numbers))]
...
while time.time() - Sim_start < Sim_end:
    ...
    if not equal:
        saturation = list(zip(range(4), degree_saturation))
        sorted_saturation = sorted(saturation, key= lambda item : item[1])
        for sat in sorted_saturation[:int(len(saturation)/4)]:
    ...
    for i in range(len(SH_numbers)):
        ...
        available_green_time = stage_lengths[i]
